...
export function preflightResultToSettings(result: PreflightResult): PreflightSettings {
  ...
  let lastMissingScopes: string[] | undefined;
  ...
  if (!result.success) {
    switch (result.errorCode) {
      ...
      case 'INSUFFICIENT_SCOPE':
        ...
        lastMissingScopes = result.missingScopes;
        break;
  ...
  return { cliVersion, lastPreflightStatus, lastPreflightTime: Date.now(), lastMissingScopes };
}
