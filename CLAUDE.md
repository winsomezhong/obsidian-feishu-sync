...
> **此节优先级最高。违反此节 = 流程违规，不可用"复杂度判断"或"效率考虑"合理化。**

### 自检规则

收到用户请求后，**在任何工具调用之前**，必须完成以下分类判定：

1. 这是否是功能性变更？（新增行为/UI/配置项/接口/错误码）
   - **是 → Path A**。禁止降级到 Path B 或 Solo。
   - 包括但不限于：UI 增强、新增显示信息、新增设置项、交互行为变更

2. 这是否是 Bug 修复或纯内部重构（不改变外部行为）？
   - **是 → Path B**

3. 这是否是纯规范维护（只改 spec/design 文档）？
   - **是 → Path C**

4. 这是否仅限拼写/格式/1-2 行修改？
   - **是 → Path D**

### 禁止行为（硬约束）

- ❌ **禁止根据"改动范围小、逻辑清晰"自行将 Path A 降级为 Path B 或 Solo**
- ❌ **禁止用 EnterPlanMode 替代 Path A 的 spec-teammate + implementer-teammate + qa-teammate 流水线**
- ❌ **禁止跳过路径判定直接开始编码**——即使需求看起来"很清楚"
- ❌ 禁止用"我觉得不需要"替代流程规则

### 违规记录

每次违规会记录到 auto-memory feedback 中。规则存在就是为了被遵守，不是被判断的。
